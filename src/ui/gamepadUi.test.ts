...
const connectedSnapshot = (): GamepadUiSnapshot => ({
  devices: [{
    index: 0,
    id: "Nintendo Co., Ltd. Pro Controller",
  ...
  sources: [{ type: "gamepad", index: 0, id: "Nintendo Co., Ltd. Pro Controller" }, { type: "keyboard" }],
...
describe("controller configuration UI", () => {
  ...
  it("switches between keyboard and controller tabs and exposes live diagnostics", () => {
    ...
    expect(root.textContent).toContain("Nintendo Switch Pro Controller");
  ...
  it("explains browser activation and reports assigned disconnects for the match pause layer", () => {
    ...
    adapter.emit();
    expect(onControllerDisconnected).toHaveBeenCalledWith(0, "Nintendo Switch Pro Controller");
    ...
    ui.showControllerReconnect(0, "Nintendo Switch Pro Controller");
