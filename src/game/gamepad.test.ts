...
describe("GamepadManager discovery and assignment", () => {
  ...
  it("handles disconnect and reconnect by id, including a changed index", () => {
    const pad = new FakePad(0, "Nintendo Switch Pro Controller");
...
describe("GamepadManager mapping and analog input", () => {
  ...
  it("uses physical Nintendo A/B labels for gameplay and menus", () => {
    const pad = new FakePad(0, "Nintendo Switch Pro Controller").button(1, true);
    const { manager } = managerWith([pad]);
    manager.poll();
